...
export default function OrderDetail({ route, navigation }: any) {
  ...
  const refundablePayment = isRefundablePayment(order);
  ...
  const cancelSummary = useMemo(() => {
    ...
    return refundablePayment
      ? "Bạn có thể hủy đơn trước giờ bắt đầu ít nhất 1 tiếng. Khoản đã thanh toán sẽ được hoàn về Ví Otto sau khi hệ thống xử lý."
      : "Bạn có thể hủy đơn trước giờ bắt đầu ít nhất 1 tiếng. Đơn sẽ dừng điều phối ngay sau khi bạn xác nhận hủy.";
  }, [cancelAllowed, refundablePayment]);
  ...
  const handleCancel = () => {
    ...
    if (!cancelAllowed) {
      Alert.alert(
        "Không thể hủy đơn",
        "Đơn hàng chỉ được hủy trước giờ bắt đầu ít nhất 1 tiếng.",
      );
      return;
    }

    const refundable = isRefundablePayment(order);
  ...
  return (
    <View style={{ flex: 1, backgroundColor: OTTO_THEME.background }}>
      ...
      >
        ...
        {cancelAllowed && cancelSummary ? (
          <SectionCard
            title="Hủy đơn nếu bạn cần thay đổi"
            subtitle={cancelSummary}
          >
            <View
              style={{
                backgroundColor: "#F8FAFC",
            ...
            >
              {[
                "Đơn chỉ được hủy khi còn cách giờ bắt đầu ít nhất 1 tiếng.",
                refundablePayment
                  ? "Nếu đơn đã thanh toán bằng ví hoặc Stripe, hệ thống sẽ hoàn tiền về Ví Otto."
                  : "Nếu chọn tiền mặt, đơn sẽ dừng điều phối ngay sau khi hủy.",
                "Tasker và thông báo liên quan sẽ được cập nhật ngay sau khi bạn xác nhận hủy.",
              ].map((item) => (
                <View
                  key={item}
                  style={{
